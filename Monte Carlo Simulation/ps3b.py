...
Simulation of the spread of virus in a patient
At every time step every viruses can either:
    - reproduce
    - mutate to resist a drug
    - become sterile due to the drug
    - perish
the faith of a virus depends on different variable probabilities
evaluated at every time-step.
Drugs can be administered to the Patient at any time.
A plot of of the virus population is made
'''
...
class Patient(object):
    ...
    def update(self):
        ...
        virusesLeft = self.viruses[:]
        for virus in virusesLeft:
            try:
...
def simulationWithDrug(numViruses, maxPop, maxBirthProb, clearProb, resistances,
    ...
    numSteps = 300
    virusTotPop   = [0] * numSteps
    virusDrugResist = [0] * numSteps
    ...
    for trial in xrange(numTrials):
        ...
        for i in xrange(numSteps):
            ...
            virusTotPop[i] += totalPop
            virusDrugResist[i] += patient.getResistPop(['guttagonol'])
    
    for i in xrange(numSteps):
        virusTotPop[i] = virusTotPop[i] / float(numTrials)
        virusDrugResist[i] = virusDrugResist[i] / float(numTrials)

    pylab.plot(virusTotPop, label="Total Population")
    pylab.plot(virusDrugResist, label="Viruses Resistant to drug")
    pylab.title("Simulation with Drug Treatment")
    pylab.xlabel("Time Step")
    pylab.ylabel("Virus Population")
    pylab.legend()
    pylab.show()
